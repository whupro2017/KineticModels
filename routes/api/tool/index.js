...
const getinfo = (req, res) => {
    ...
    const arr = [item_id];
    db.query(tool.queryById, arr, function (err, rows) {
        if (err) {
            RspMsg(res, {
                data: '失败',
                msg: 'error',
...
const add = (req, res) => {
    ...
    db.query(tool.insert, arr, function (err, rows) {
        if (err) {
            RspMsg(res, {
                data: '失败',
                msg: 'error',
...
const update = (req, res) => {
    const v1 = req.body;
    const arr = [
        v1.PRINT_TYPE, v1.COLLECTED_BY_NAME, v1.TOOL_JUDGEMENT,
        v1.DESCRIPTION, v1.LEFT_POSITION, v1.COLLECTION_MODE,
        v1.COLLECTED_DATE, v1.UTILIZATION, v1.CRIMINAL_FLAG, v1.ID
    ];
    db.query(tool.updateInfo, arr, (err, rows) => {
        if (err) {
            RspMsg(res, {
                data: '失败',
                msg: 'error',
